...
const handleScheduleClick = async () => {
  ...
  try {
    if (user?.role === 'admin') {
      ...
      // Fetch Doctors
      const docsRes = await api.get('/doctors'); // Assuming this endpoint exists or similar
      const docs = docsRes.doctors || []
      setDoctorsList(docs)

      // Fetch All Patients (via bookings or patients endpoint)
      const patRes = await api.get(`/admin/patients?admin_token=${token}`)
      setPatientsList(patRes.patients || [])
    } else if (user?.role === 'doctor') {
      // Fetch Patients from bookings
      const res = await api.get(`/bookings?doctor_name=${user.name}`)
      if (res.booking || res.bookings) {
        const bookings = res.booking || res.bookings
        const uniquePatients = [...new Set(bookings.map(b => b.name || b.patient_name))]
        setPatientsList(uniquePatients)
      }
      setFollowUpData(prev => ({ ...prev, doctorName: user.name, doctorId: user.id }))
    } else {
      // Patient role -> Redirect as before
      window.location.href = '/dashboard/contact-doctor'
    }
  } catch (e) {
    console.error(e)
  } finally {
...
const handleBookFollowUp = async () => {
  try {
    if (!followUpData.patientName || !followUpData.date || !followUpData.time) {
      alert('Please fill all fields')
      return
    }
    if (user.role === 'admin' && !followUpData.doctorId) {
      alert('Please select a doctor')
      return
    }

    const params = new URLSearchParams({
      patient_name: followUpData.patientName,
      doctor_name: followUpData.doctorName, // For doctor role this is set on open, for admin selected
      date: followUpData.date,
      time: followUpData.time,
      status: 'Confirmed' // Direct confirmation for follow-up
    })
    ...
    if (res.success) {
      ...
      setFollowUpData({ doctorId: '', doctorName: '', patientName: '', date: '', time: '' })
    } else {
        ...
        <div className="card border-0 shadow-sm">
          ...
          <div className="card-body">
            <div className="d-grid gap-3">
              ...
              <button className="btn btn-outline-secondary" onClick={handleScheduleClick}>
        ...
        <div className="modal-dialog modal-dialog-centered">
          <div className="modal-content">
            <div className="modal-header">
              ...
              <button type="button" className="btn-close" onClick={() => setIsFollowUpModalOpen(false)}></button>
            </div>
            <div className="modal-body">
              {followUpLoading ? (
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          setFollowUpData({ ...followUpData, doctorId: e.target.value, doctorName: doc?.name || '' })
                        }}
                      >
                  ...
                  {user?.role !== 'patient' && (
                    <div>
                      ...
                      >
                        ...
                        {typeof patientsList[0] === 'string'
                          ? patientsList.map((p, i) => <option key={i} value={p}>{p}</option>)
            ...
            <div className="modal-footer">
              ...
              <button type="button" className="btn btn-primary" onClick={handleBookFollowUp}>Confirm Booking</button>
            </div>
          </div>
        </div>
      </div>
