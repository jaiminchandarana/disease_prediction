...
const PatientBookings = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        let url = '/bookings'
        if (user?.role === 'doctor') {
          url += `?doctor_name=${encodeURIComponent(user.name)}`
        } else if (user?.role === 'admin') {
          const token = localStorage.getItem('token')
          url = `/admin/bookings?admin_token=${token}`
        }
        ...
        if (res.success && mounted) {
          const mapped = (res.bookings || []).map(b => {
            ...
            const statusCap = (b.status || 'pending')
              .toLowerCase()
  ...
  const handleStatusChange = (bookingId, newStatus) => {
    const doUpdate = async () => {
      try {
        await api.get(`/bookings/update-status?booking_id=${bookingId}&status=${encodeURIComponent(newStatus)}`)
  ...
  const handleDelete = (id) => {
    if (window.confirm('Are you sure you want to delete this booking?')) {
      const doDelete = async () => {
        try {
          await api.get(`/bookings/delete?booking_id=${id}`)
          ...
          <div className="p-3">
            <div className="d-flex align-items-center mb-4">
              ...
              <div>
                ...
                <small className="text-muted">{user?.role === 'doctor' ? 'Doctor ID' : 'Admin ID'}: {user?.id || '---'}</small>
